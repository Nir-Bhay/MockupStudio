...
function setLayout(k) {
  ...
  // Force reposition when user explicitly selects a layout
  applyLayout(true);
}
...
function applyLayout(force) {
  ...
  const p = S.pad;
  const reposition = force || !S.freeMode;
  ...
  // Auto-refresh bezels when layout changes (watch/laptop/monitor need different bezel types)
  if (typeof refreshBezels === 'function') refreshBezels();
}
