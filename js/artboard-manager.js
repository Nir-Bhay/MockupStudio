...
function _snapshotArtboard() {
  return {
    ...
    freeMode: S.freeMode,
    imgFit: _deepClone(S.imgFit),
    imgScale: _deepClone(S.imgScale),
    imgOff: _deepClone(S.imgOff),
    imgRad: _deepClone(S.imgRad),
    imgRotation: _deepClone(S.imgRotation),
    imgOpacity: _deepClone(S.imgOpacity),
    imgFlip: _deepClone(S.imgFlip),
    imgFilters: _deepClone(S.imgFilters),
...
function _loadArtboard(ab) {
  ...
  S.imgFit = _deepClone(ab.imgFit);
  S.imgScale = _deepClone(ab.imgScale);
  S.imgOff = _deepClone(ab.imgOff);
  S.imgRad = _deepClone(ab.imgRad);
  S.imgRotation = _deepClone(ab.imgRotation);
  S.imgOpacity = _deepClone(ab.imgOpacity);
  S.imgFlip = _deepClone(ab.imgFlip);
  S.imgFilters = _deepClone(ab.imgFilters);
  S.texts = _deepClone(ab.texts || []);
  S.shapes = _deepClone(ab.shapes || []);
  S.annotations = _deepClone(ab.annotations || []);
  S.badges = _deepClone(ab.badges || []);
  S.noiseEnabled = ab.noiseEnabled || false; S.noiseIntensity = ab.noiseIntensity || 15;
  S.vignetteEnabled = ab.vignetteEnabled || false; S.vignetteIntensity = ab.vignetteIntensity || 40;
...
function _renderArtboardVisuals() {
  ...
  // Restore ALL dynamic elements (texts, shapes, annotations, badges, noise, vignette)
  _restoreDynamic();
...
async function batchExport() {
  ...
  try {
    for (let i = 0; i < S.artboards.length; i++) {
      ...
      $('expOv').querySelector('.exp-t').textContent = 'Exporting ' + (i + 1) + '/' + S.artboards.length + '...';
